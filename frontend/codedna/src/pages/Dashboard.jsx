  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 auto-rows-[minmax(150px,auto)]">
        ...
        <div className="bg-base-200 border border-base-300 rounded-box p-6 flex flex-col items-center justify-center gap-2">
          <div className="radial-progress text-secondary" style={{ "--value": confidence, "--size": "5rem" }} role="progressbar">
            {confidence}%
          </div>
          <span className="text-sm opacity-60">AI Confidence</span>
        </div>
        ...
        <Link to={`/timeline/${analysisId}`} className="bg-base-200 border border-base-300 rounded-box p-6 flex flex-col justify-between hover:border-primary transition-colors">
          <span className="font-display font-semibold">Timeline</span>
          <span className="text-sm opacity-60">Track evolution over time →</span>
        </Link>
      </div>
    </div>
  );
