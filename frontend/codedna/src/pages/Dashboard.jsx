...
import { Network, ShieldAlert, Wrench, History, ArrowRight } from "lucide-react";
...
const Dashboard = () => {
  ...
  const confidence = Math.round((riskModules?.average_confidence ?? 0) * 100);
  ...
  return (
    <div className="flex flex-col gap-5 sm:gap-6">
      <div>
        <h1 className="font-display text-xl sm:text-2xl font-semibold text-base-content">{repository?.name}</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-5 auto-rows-[minmax(150px,auto)]">
        ...
        <div className="bg-linear-to-br from-base-100 to-base-200 shadow-clay rounded-[1.5rem] p-5 sm:p-6 flex flex-col items-center justify-center gap-3">
          <div className="rounded-full shadow-clay-pressed p-2 bg-base-200">
            <div
              ...
              style={{ "--value": confidence, "--size": "4.5rem", "--thickness": "5px" }}
              role="progressbar"
            >
              <span className="text-sm font-semibold">{confidence}%</span>
            </div>
          </div>
          <span className="text-xs sm:text-sm text-base-content/55 text-center">AI Confidence</span>
        </div>
        ...
        <div className="bg-linear-to-br from-base-100 to-base-200 shadow-clay rounded-[1.5rem] p-5 sm:p-6 flex flex-col items-center justify-center gap-2">
          <span className="text-3xl sm:text-4xl font-display font-semibold text-error">{highRiskCount}</span>
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Network size={17} className="text-secondary/60" strokeWidth={2} />
          </div>
          <span className="text-sm text-base-content/55 flex items-center gap-1.5 group-hover:text-secondary transition-colors duration-200">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <ShieldAlert size={17} className="text-error/60" strokeWidth={2} />
          </div>
          <span className="text-sm text-base-content/55 flex items-center gap-1.5 group-hover:text-error transition-colors duration-200">
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <Wrench size={17} className="text-warning/70" strokeWidth={2} />
          </div>
          <span className="text-sm text-base-content/55 flex items-center gap-1.5 group-hover:text-warning transition-colors duration-200">
        ...
        <Link
          to={`/timeline/${analysisId}`}
          className="group bg-linear-to-br from-base-100 to-base-200 shadow-clay-sm hover:shadow-clay rounded-[1.5rem] p-5 sm:p-6 flex flex-col justify-between gap-6 transition-all duration-200 hover:-translate-y-0.5 cursor-pointer"
        >
          <div className="flex items-center justify-between">
            <span className="font-display font-semibold text-base-content">Timeline</span>
            <History size={17} className="text-info/70" strokeWidth={2} />
          </div>
          <span className="text-sm text-base-content/55 flex items-center gap-1.5 group-hover:text-info transition-colors duration-200">
            Track evolution over time <ArrowRight size={14} className="group-hover:translate-x-0.5 transition-transform duration-200" />
          </span>
        </Link>
      </div>
    </div>
