...
const markdownComponents = {
  ...
  p: ({ children }) => <p className="text-sm opacity-80 leading-relaxed mb-3">{children}</p>,
  ul: ({ children }) => <ul className="list-disc list-inside text-sm opacity-80 flex flex-col gap-1 mb-3">{children}</ul>,
  ol: ({ children }) => <ol className="list-decimal list-inside text-sm opacity-80 flex flex-col gap-1 mb-3">{children}</ol>,
  li: ({ children }) => <li>{children}</li>,
  pre: ({ children }) => (
    <pre className="font-mono text-xs bg-base-300 p-3 rounded-box overflow-x-auto mb-3">{children}</pre>
  ),
  ...
  if (!documentation) {
    return (
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm opacity-60">No documentation was generated for this analysis.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl">
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="font-display text-2xl font-semibold">Report</h1>
          <p className="text-sm opacity-60">AI-generated documentation for {repository?.name}</p>
        </div>
      ...
      <div role="tablist" className="tabs tabs-boxed w-fit">
        <button role="tab" onClick={() => setActiveTab("overview")} className={`tab ${activeTab === "overview" ? "tab-active" : ""}`}>
          Overview
        </button>
        <button role="tab" onClick={() => setActiveTab("readme")} className={`tab ${activeTab === "readme" ? "tab-active" : ""}`}>
          README
        </button>
        <button role="tab" onClick={() => setActiveTab("insights")} className={`tab ${activeTab === "insights" ? "tab-active" : ""}`}>
          Insights
        </button>
        <button role="tab" onClick={() => setActiveTab("recommendations")} className={`tab ${activeTab === "recommendations" ? "tab-active" : ""}`}>
          Recommendations
        </button>
      </div>
      ...
      <div className="bg-base-200 border border-base-300 rounded-box p-6">
        {activeTab === "overview" && (
          <div className="flex flex-col gap-6">
            <div>
              ...
              <p className="text-sm opacity-80 leading-relaxed">{documentation.summary || "Not available."}</p>
            </div>
            <div>
              <h3 className="font-display text-sm font-semibold text-primary mb-2">Architecture Overview</h3>
              <p className="text-sm opacity-80 leading-relaxed">{documentation.architecture_overview || "Not available."}</p>
            </div>
            <div>
              <h3 className="font-display text-sm font-semibold text-primary mb-2">Quality Assessment</h3>
              <p className="text-sm opacity-80 leading-relaxed">{documentation.quality_assessment || "Not available."}</p>
            </div>
          </div>
        )}
        ...
        {activeTab === "readme" &&
          (documentation.readme ? (
            ...
            <p className="text-sm opacity-60">No README content was generated.</p>
          ))}
        ...
        {activeTab === "recommendations" && (
          <p className="text-sm opacity-80 leading-relaxed">{documentation.recommendations || "No recommendations were generated."}</p>
        )}
      </div>
    </div>
  );
