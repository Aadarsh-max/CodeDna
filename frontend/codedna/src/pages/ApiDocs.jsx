...
const markdownComponents = {
  ...
  pre: ({ children }) => (
    <pre className="font-mono text-xs bg-base-300 p-3 rounded-lg overflow-x-auto mb-3 shadow-clay-pressed">{children}</pre>
  ),
  ...
  if (!apiDocumentation || apiDocumentation.total_routes === 0) {
    return (
      <div className="flex flex-col gap-2">
        ...
        <p className="text-sm opacity-60">No API routes were detected to document for this analysis.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl">
      ...
      <div className="bg-base-200 rounded-box shadow-clay p-6">
        <ReactMarkdown components={markdownComponents}>{apiDocumentation.api_reference}</ReactMarkdown>
      </div>
    </div>
  );
};
