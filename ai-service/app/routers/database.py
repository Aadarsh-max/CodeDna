from fastapi import APIRouter, Request
from app.schemas.repo_schema import ParseRequest
from app.schemas.database_schema import DatabaseSchemaResponse
from app.services.parser_service import read_repository_files
from app.services.database_service import detect_database_schema
from app.utils.rate_limiter import limiter

router = APIRouter()


@router.post("/database", response_model=DatabaseSchemaResponse)
@limiter.limit("15/minute")
def scan_database(request: Request, payload: ParseRequest):
    file_contents = read_repository_files(
        source=payload.source,
        github_url=payload.github_url,
        zip_path=payload.zip_path,
    )
    return detect_database_schema(file_contents)